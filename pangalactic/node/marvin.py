...
# -*- coding: utf-8 -*-
"""
Marvin (the PanGalactic caching client) main window
"""
...
class Main(QMainWindow):
    """
    Main window of the 'marvin' client gui.
    ...
    def __init__(self, home='', test_data=None, width=None, height=None,
        ...
        # NOTE: this probably is not doing what it is supposed to do as it's
        # written right now, because (per Qt docs): "The objectName property is
        # used to identify each QToolBar and QDockWidget. You should make sure
        ...
        self.add_splash_msg('... db initialized ...')
    ...
    def get_or_create_local_user(self):
        ...
        if local_user:
            ...
            local_user = clone('Person', oid='me')
            local_user.id = 'me'
            local_user.name = 'Me'
            self.local_user = local_user
    ...
    def on_rpc_get_user_roles_result(self, data):
        ...
        if szd_user:
            ...
            orb.log.debug(f' - local user returned: "{user_id}"')
            state['local_user_oid'] = str(self.local_user.oid)
            if str(state.get('local_user_oid')) == 'me':
                # current local user is 'me' -- replace ...
                uid_msg = f'local user was "me", replacing with "{user_id}"'
                orb.log.debug(f' - {uid_msg} ...')
                state['local_user_oid'] = str(self.local_user.oid)
                # me = orb.get('me')
                ...
                log_msg = f'login user "{user_id}" matches local user.'
                orb.log.debug(f'    + {log_msg}')
        ...
        if invalid_orgs:
            orb.log.debug(f'    deleting {len(invalid_orgs)} invalid orgs.')
        ...
        # [0] delete any role assignments whose oids are unknown to the server
        orb.log.debug('    checking for role assignments in local db ...')
        ras_local = orb.get_by_type('RoleAssignment')
        orb.log.debug(f'      {len(ras_local)} found ...')
        invalid_ras = [ra for ra in ras_local if ra.oid in bad_oids]
        if invalid_ras:
            orb.log.debug(f'      {len(invalid_ras)} invalid ones, deleting ...')
            if invalid_ras:
                orb.delete(invalid_ras)
        else:
            orb.log.debug('      no invalid ones found.')
        # [1] deserialize RoleAssignment objects we don't have
        new_ras = []
        for sra in szd_ras:
            if sra['oid'] not in matrix:
                new_ras.append(sra)
        if new_ras:
            orb.log.debug(f'      {len(new_ras)} not in matrix, adding ...')
            orb.log.debug(f'      {new_ras}')
            deserialize(orb, new_ras)
        # [2] refresh the 'orb.user_raz' cache
        orb.log.debug('* refreshing orb.user_raz ...')
        updated_ras_local = orb.get_by_type('RoleAssignment')
        local_user_ras = [ra for ra in updated_ras_local
                          if ra.assigned_to is self.local_user]
        orb.log.debug(f'  {len(local_user_ras)} local user ras found ...')
        ids = [ra.id for ra in local_user_ras]
        orb.log.debug(f'  {ids}')
        ...
        if admins:
            ...
            # also, deserialize ALL role_assignments that have that org/project
            # as their "role_assignment_context", since the user will need them
            # in the admin tool when assigning roles in those orgs
            admin_org_oids = [ra.get('role_assignment_context') for ra in admins
                              if ra.get('role_assignment_context') is not None]
            admin_org_ras = [ra for ra in szd_ras
                             if ra.get('role_assignment_context') in
                             admin_org_oids]
            deserialize(orb, admin_org_ras)
        orb.log.info('    channels we will subscribe to: {}'.format(
                                                       str(self.channels)))
        if self.project:
            p_oid = self.project.oid
            proj_ras = [ra for ra in orb.user_raz
                        if ra.get('role_assignment_context') == p_oid]
            roles = [orb.get(ra.get('assigned_role')) for ra in proj_ras
                     if ra.get('assigned_role') is not None]
            my_roles = [role.name for role in roles]
            if my_roles:
    ...
    def sync_user_created_objs_to_repo(self, data):
        ...
        oids = [o.oid for o in self.local_user.created_objects
                if ((o._cname == 'ProjectSystemUsage')
    ...
    def sync_library_objs(self, data):
        """
        Sync with the repository's "public" objects. This updates all "public"
    ...
    def sync_current_project(self, data):
        ...
        if (proj_oid != 'pgefobjects:SANDBOX') and project:
            ...
            self.statusbar.showMessage('syncing project {} ...'.format(
                                                                 project.id))
            # ****************************************************************
            # NOTE: new approach (as of 2022-08-20 SCW) -- don't send any data
            # to the server, have the server send *all* project objects and let
            # the deserialization process sort out which ones are newer ...
            # ****************************************************************
    ...
    def on_sync_result(self, data, project_sync=False, user_objs_sync=False):
        ...
        if project_sync:
            ...
            sync_type = 'user objs'
        # orb.log.debug('       data: {}'.format(str(data)))
        # NOTE:  for marvin, "local_only" objects are not possible since
        # objects are only created by marvin when connected to the server
        try:
        ...
        if local_only:
            ...
            for o in local_only_objs:
                if hasattr(o, 'creator') and o.creator == self.local_user:
                    if not (o._cname == 'ProjectSystemUsage' and
    ...
    def on_sync_library_result(self, data, project_sync=False):
        ...
        if local_only:
            # In Marvin, NO objects should be local only!
            orb.log.debug('  objects unknown to server found ...')
    ...
    def on_get_library_objects_result(self, data):
        ...
        if data is not None:
            msg = f'deserializing {len(data)} objects ...'
            orb.log.debug(f'  - {msg}')
            self.statusbar.showMessage(msg)
            deserialize(orb, data)
                        ...
                        for obj in objs:
                            if orb.is_a(obj, 'Person'):
                                ...
                                # dispatcher.send('person added', obj=obj,
                                                ...
                                                # pk_added=pk_added)
                            elif orb.is_a(obj, 'Organization'):
    ...
    def on_received_objects(self, content=None):
        ...
        if serialized_objects:
            ...
            if n:
                so = serialized_objects[0]
                orb.log.debug("  first serialized object:")
                orb.log.debug(f'  {pprint.pformat(so)}')
        else:
            orb.log.debug('  content was empty!')
            return False
        ...
        for obj in objs:
            ...
            if cname == 'ParameterDefinition':
                ...
                if getattr(obj, 'project', None):
                    if obj.project is self.sandbox:
    ...
    def _create_actions(self):
        ...
        app_name = config.get('app_name', 'Marvin'),
    ...
    def projects(self):
        ...
        try:
            ...
            if global_admin:
                ...
                orb.log.debug(f'  - local user "{userid}" is NOT Global Admin')
                # if user is not a global admin, restrict the projects to those
                # on which the user has a role
                proj_oids = [o.oid for o in orb.get_by_type('Project')]
                projects = set([orb.get(ra.get('role_assignment_context'))
                            for ra in orb.user_raz
                            if ra.get('role_assignment_context') in proj_oids])
    ...
    def systems(self):
        ...
        return [orb.get(system.oid) for system in systemz.get(project.oid)]
    ...
    def set_product(self, p):
        ...
        """
        p_id = getattr(p, 'id', '(no id)') or '(no id)'
        orb.log.debug(f'* set_product({p_id})')
        if not p:
    ...
    def on_comp_back(self, oid=None):
        ...
        from the stack.
        """
        orb.log.debug('* "back" was called in component modeler')
        if state.get('component_modeler_history'):
    ...
    def init_toolbar(self):
        ...
        self.delete_project_action.setVisible(False)
        self.delete_project_action.setEnabled(False)
        self.new_project_action.setVisible(True)
    ...
    def on_remote_deleted_signal(self, content=None):
        ...
        if obj:
            ...
            selected_sys_oid = state.get('system', {}).get(
                                                        state.get('project'))
            cname = obj.__class__.__name__
            ...
            if cname in ['Acu', 'ProjectSystemUsage', 'HardwareProduct']:
                ...
                if selected_sys_oid == relevant_obj_oid:
                    ...
                    if hasattr(self, 'system_model_window'):
                        try:
                            orb.log.info('  setting diagram subj. to project')
    ...
    def on_remote_get_mod_object(self, ser_objs):
        ...
        for n, obj in enumerate(objs):
            ...
            if cname == 'RoleAssignment':
                ...
                self.library_widget.refresh(cname=cname)
            if getattr(self, 'sys_tree', None) and self.mode == 'system':
                orb.log.debug('  updating system tree ...')
                if cname in ['HardwareProduct', 'Acu', 'ProjectSystemUsage']:
    ...
    def on_mod_object_signal(self, obj=None, cname='', new=False):
        ...
        if obj:
            ...
            # NOTE: self.library_widget refreshes itself (it listens for "new
            # object"/"modified object"), so no need to refresh it
            # NOTE: no need to do anything in 'db' mode -- the object table
            # listens for the 'mod object' signal and handles it ...
            if (getattr(self, 'sys_tree', None) and self.mode == 'system'
                and cname in ['HardwareProduct', 'Acu', 'ProjectSystemUsage']):
            ...
            if (state.get('connected')
                ...
                if cname == 'RoleAssignment':
                    ...
                    orb.log.debug('  - saved obj id: {} | oid: {}'.format(
                                                          obj.id, obj.oid))
                    orb.log.debug('  - serialized as:')
                    orb.log.debug(f'    {pprint.pformat(serialized_objs)}')
    ...
    def on_vger_save_result(self, stuff):
        ...
        try:
            ...
            if not msg:
                ...
                msg = 'synced.'
                orb.log.debug(f'* {msg}')
            self.statusbar.showMessage('almost synced ...')
            orb.log.debug('vger save: {}'.format(msg))
            self.get_vger_caches()
        except:
            orb.log.debug('  result format incorrect.')
            self.statusbar.showMessage('almost synced ...')
            self.get_vger_caches()

    def get_vger_caches(self):
        """
        Get the server caches for the client's current objects:  'componentz',
        'systemz', 'parm_defz', 'de_defz', 'parmz_by_dimz', 'rqt_allocz'.
        """
        # TODO:  fix scalability issue with getting unlimited 'componentz'
        oids = list(matrix)
        rpc = self.mbus.session.call('vger.get_caches', oids=oids)
        rpc.addCallback(self.on_get_vger_caches_result)
        rpc.addErrback(self.on_get_vger_caches_failure)

    def on_get_vger_caches_result(self, data):
        orb.log.debug('- on_get_vger_caches_result()')
        try:
            ser_compz, ser_systemz, pdz, ddz, pbdz, rqtaz, az = data
            componentz.clear()
            componentz.update(deserialize_compz(ser_compz))
            systemz.clear()
            systemz.update(deserialize_systemz(ser_systemz))
            # parm_defz, de_defz, parmz_by_dimz, and rqt_allocz are complete
            # data, so assign them directly to the caches
            parm_defz.clear()
            parm_defz.update(pdz)
            de_defz.clear()
            de_defz.update(ddz)
            parmz_by_dimz.clear()
            parmz_by_dimz.update(pbdz)
            rqt_allocz.clear()
            rqt_allocz.update(deserialize_rqt_allocz(rqtaz))
            allocz.clear()
            allocz.update(az)
            self.statusbar.showMessage('synced.')
            names = "componentz, parm_defz, de_defz, parmz_by_dimz, rqt_allocz"
            names += ", allocz"
            orb.log.debug(f'* got vger caches: {names}.')
            # tree & dashboard may need refreshing from new componentz/systemz
            if state.get('mode') == 'system' and hasattr(self, 'sys_tree'):
                self.refresh_tree_and_dashboard()
            # for intensive debugging only -- extremely verbose!
            # orb.log.debug(f'  - ser_compz: {ser_compz}.')
            # orb.log.debug(f'  - pdz: {pdz}.')
            # orb.log.debug(f'  - ddz: {ddz}.')
            # orb.log.debug(f'  - pbdz: {pbdz}.')
            # orb.log.debug(f'  - rqtaz: {rqtaz}.')
            # orb.log.debug(f'  - az: {az}.')
        except:
            orb.log.debug('  result format incorrect.')
            self.statusbar.showMessage('synced.')

    def on_get_vger_caches_failure(self, f):
        orb.log.debug('* rpc vger.get_caches failure: {}'.format(
                      f.getTraceback()))
    ...
    def on_deleted_object_signal(self, oid='', cname='', remote=False):
        """
        Call functions to update applicable widgets when object(s) have been
        deleted, which is always performed on the server side.
    ...
    def update_project_role_labels(self):
        ...
        if global_admin:
            ...
            orb.log.debug(f'* local user "{userid}" is NOT a global admin.')
        if p:
            ...
            if p.id == 'SANDBOX':
                ...
                p_ras = [ra for ra in orb.user_raz
                         if ra.get('role_assignment_context') == p.oid]
                p_role_objs = [orb.get(ra.get('assigned_role')) for ra in p_ras
                               if ra.get('assigned_role') is not None]
                p_roles = [role.name for role in p_role_objs]
                admin_oid = 'pgefobjects:Role.Administrator'
                project_admin = bool([ra for ra in orb.user_raz
                                  if ra.get('assigned_role') == admin_oid and
                                  ra.get('role_assignment_context') == p.oid])
                ...
                if state['connected']:
                    creator_oid = getattr(p.creator, 'oid', '')
                    # anyone can create a new project, when connected
                    self.new_project_action.setEnabled(True)
                    self.new_project_action.setVisible(True)
                    if ((creator_oid == self.local_user.oid)
                          or global_admin):
                    ...
                    self.modes_def_action.setVisible(False)
                    self.modes_def_action.setEnabled(False)
                    # projects cannot be deleted when offline
                    self.delete_project_action.setEnabled(False)
                    self.delete_project_action.setVisible(False)
        else:
    ...
    def refresh_tree_views(self, rebuilding=False, selected_link_oid=None):
        ...
        systems = [orb.get(system.oid)
                   for system in (systemz.get(self.project.oid) or [])]
        for system in systems:
            cycles = orb.check_for_cycles(system)
            if cycles:
    ...
    ### SET UP 'data' mode
    # NOTE: "DataGrid" has been deprecated, but "data" mode may be resurrected
    # later for data import, using a different "spreadsheet" widget
    ...
    def delete_project(self):
        ...
        self.delete_objects([project_oid])

    def delete_objects(self, oids):
        """
        Request that the server delete a set of objects.

        Args:
            oids (list of str): oids of objects to be deleted
        """
        if state.get('connected'):
            orb.log.info('  - calling "vger.delete"')
            rpc = self.mbus.session.call('vger.delete', oids)
            rpc.addCallback(self.on_deletion_result)
            rpc.addErrback(self.on_failure)

    def on_deletion_result(self, res):
        """
        Handle result of a delete operation.
        """
        orb.log.debug('* on_deletion_result()')
        oids_not_found, oids_deleted = [[], []]
        try:
            oids_not_found, oids_deleted = res
        except:
            orb.log.info('  - result format was incorrect.')
        orb.log.debug(f'  - oids deleted: "{oids_deleted}"')
        for oid in oids_deleted:
            # first check if we have the object
            obj = orb.get(oid or '')
            if obj:
                orb.log.debug(f'  - "{obj.id}" exists locally')
                # if deleted object was the selected system, set selected system
                # and diagram subject to the project and refresh the diagram
                selected_sys_oid = state.get('system', {}).get(
                                                    state.get('project'))
                ...
                if cname in ['Acu', 'ProjectSystemUsage', 'HardwareProduct']:
                    ...
                    if selected_sys_oid == relevant_obj_oid:
                        if (state.get('component_modeler_history') and
                        relevant_obj_oid in state['component_modeler_history']):
                            state['component_modeler_history'].remove(oid)
                        orb.log.info('  deleted object was selected system')
                        state['system'][state['project']] = state['project']
                        if hasattr(self, 'system_model_window'):
                            try:
                ...
                orb.log.debug('  oid not found in local db; ignoring.')
                # but just to be on the safe side, make sure the oid is gone
                # from the matrix too
                orb.delete([oid])
    ...
    def new_product_wizard(self):
        ...
        instances.
        """
        orb.log.debug('* there is no new_product_wizard()')
    ...
    def load_serialized_objects(self, sobjs, importing=False):
        ...
        if sobjs:
            ...
            self.statusbar.showMessage(msg)
            cls_names = orb.get_subclass_names('Product')
            cls_names |= set(['Acu', 'ProjectSystemUsage'])
            new_products_psus_or_acus = [obj for obj in objs
                                     if obj.__class__.__name__ in cls_names]
            if new_products_psus_or_acus:
                # orb.recompute_parmz()
                if hasattr(self, 'library_widget'):
                    self.library_widget.refresh()
                if hasattr(self, 'sys_tree') and self.mode == 'system':
    ...
    def force_load_serialized_objects(self, sobjs, importing=False):
        ...
        if sobjs:
            ...
            self.statusbar.showMessage(msg)
            cls_names = orb.get_subclass_names('Product')
            cls_names += ['Acu', 'ProjectSystemUsage']
            new_products_psus_or_acus = [obj for obj in objs
                                     if obj.__class__.__name__ in cls_names]
            if new_products_psus_or_acus:
                # orb.recompute_parmz()
    ...
    def load_test_objects(self):
        if not state.get('test_objects_loaded'):
            ...
            self.load_serialized_objects(sobjs)
            all_hw = orb.get_by_type('HardwareProduct')
            hw = [h for h in all_hw if h.id_ns == 'test']
            orb.assign_test_parameters(hw)
            all_rf = orb.get_by_type('RepresentationFile')
            rfs = [rf for rf in all_rf if rf.id_ns == 'test']
            for rf in rfs:
    ...
    # NOTE:  need to create "marv" versions of wizards ...
    # ----------------------------------------------------------------
    # THIS SECTION COMMENTED OUT UNTIL "MARV" WIZARDS ARE CREATED ...
    # ----------------------------------------------------------------
...
def cleanup_and_save():
    orb.log.debug('* writing config, prefs, and state files ...')
